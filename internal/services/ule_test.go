...
func TestULEServiceContractShape(t *testing.T) {
	article := repository.Article{ID:"articulo-001",Titulo:"Título",Fecha:time.Date(2026,9,22,0,0,0,0,time.UTC),Resumen:"Resumen",ContenidoHTML:"<p>ok</p>",Etiquetas:[]string{"uno"},Visible:true,BibliographyIDs:[]string{"biblio-001"}}
	...
	ad := repository.Ad{ID:"ad-001",Tipo:"evento",Activo:true,Peso:1,Inicio:ptrTime(time.Date(2026,9,1,0,0,0,0,time.UTC)),Fin:ptrTime(time.Date(2026,10,1,0,0,0,0,time.UTC))}
