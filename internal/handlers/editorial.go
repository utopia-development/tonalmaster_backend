...
func(h *EditorialHandler)CreateCatalog(w http.ResponseWriter,r *http.Request){u,ok:=h.auth(w,r);if !ok{return};var q catalogRequest;if json.NewDecoder(r.Body).Decode(&q)!=nil||q.ID==""||q.Titulo==""{writeError(w,400,"invalid_request","id and titulo are required");return};if len(q.Detalles)==0{q.Detalles=[]byte("{}")};x,e:=h.s.CreateCatalog(r.Context(),u.Role,repository.Catalog{ID:q.ID,Titulo:q.Titulo,Descripcion:q.Descripcion,ImagenPortada:q.ImagenPortada,Detalles:q.Detalles,Visible:q.Visible});if e!=nil{writeError(w,400,"catalog_create_failed","could not create catalog");return};writeJSON(w,201,catalogResponse(x,nil))}
func(h *EditorialHandler)UpdateCatalog(w http.ResponseWriter,r *http.Request){u,ok:=h.auth(w,r);if !ok{return};var q catalogRequest;if json.NewDecoder(r.Body).Decode(&q)!=nil{writeError(w,400,"invalid_request","invalid JSON");return};if len(q.Detalles)==0{q.Detalles=[]byte("{}")};x,e:=h.s.UpdateCatalog(r.Context(),u.Role,r.PathValue("id"),repository.Catalog{ID:r.PathValue("id"),Titulo:q.Titulo,Descripcion:q.Descripcion,ImagenPortada:q.ImagenPortada,Detalles:q.Detalles,Visible:q.Visible});if e==repository.ErrNotFound{writeError(w,404,"catalog_not_found","not found");return};if e!=nil{writeError(w,400,"catalog_update_failed","could not update");return};writeJSON(w,200,catalogResponse(x,nil))}
...
func articleResponse(x repository.Article) articleResponse { return articleResponse{ID:x.ID,Titulo:x.Titulo,Autor:x.Autor,Fecha:x.Fecha.Format("2006-01-02"),Resumen:x.Resumen,ContenidoHTML:x.ContenidoHTML,ImagenDestacada:x.ImagenDestacada,ImagenAlt:x.ImagenAlt,Categoria:x.Categoria,Etiquetas:x.Etiquetas,BibliographyIDs:x.BibliographyIDs,Visible:x.Visible} }

type bibliographyResponse struct { ID string `json:"id"`; Titulo string `json:"titulo"`; Autores []string `json:"autores"`; Anio *int `json:"año"`; Tipo string `json:"tipo"`; Editorial *string `json:"editorial"`; Resumen *string `json:"resumen"`; URL *string `json:"url"`; ArticleIDs []string `json:"articulos_relacionados"`; Visible bool `json:"visible"` }
func bibliographyResponse(x repository.Bibliography) bibliographyResponse { return bibliographyResponse{ID:x.ID,Titulo:x.Titulo,Autores:x.Autores,Anio:x.Anio,Tipo:x.Tipo,Editorial:x.Editorial,Resumen:x.Resumen,URL:x.URL,ArticleIDs:x.ArticleIDs,Visible:x.Visible} }

type catalogResponse struct { ID string `json:"id"`; Titulo string `json:"titulo"`; Descripcion *string `json:"descripcion"`; ImagenPortada *string `json:"imagen_portada"`; CategoriasDisponibles map[string]any `json:"categorias_disponibles"`; Elementos []itemResponse `json:"elementos"`; Visible bool `json:"visible"` }
func catalogResponse(x repository.Catalog, items []repository.CatalogItem) catalogResponse { var d map[string]any; _=json.Unmarshal(x.Detalles,&d); cats:=map[string]any{};if v,ok:=d["categorias_disponibles"].(map[string]any);ok{cats=v};out:=make([]itemResponse,0,len(items));for _,i:=range items{out=append(out,itemResponse(i))};return catalogResponse{ID:x.ID,Titulo:x.Titulo,Descripcion:x.Descripcion,ImagenPortada:x.ImagenPortada,CategoriasDisponibles:cats,Elementos:out,Visible:x.Visible} }

type itemResponse struct { ID string `json:"id"`; Titulo string `json:"titulo"`; Imagen string `json:"imagen"`; ImagenAlt *string `json:"imagen_alt"`; Descripcion *string `json:"descripcion"`; Categorias map[string]any `json:"categorias"`; AnioDescubrimiento *int `json:"año_descubrimiento"`; Ubicacion *string `json:"ubicacion"` }
func itemResponse(x repository.CatalogItem) itemResponse { var d map[string]any;_=json.Unmarshal(x.Detalles,&d); out:=itemResponse{ID:x.ID,Titulo:x.Titulo,Imagen:x.Imagen}; if v,ok:=d["imagen_alt"].(string);ok{out.ImagenAlt=&v};if v,ok:=d["descripcion"].(string);ok{out.Descripcion=&v};if v,ok:=d["categorias"].(map[string]any);ok{out.Categorias=v};if v,ok:=d["año_descubrimiento"].(float64);ok{n:=int(v);out.AnioDescubrimiento=&n};if v,ok:=d["ubicacion"].(string);ok{out.Ubicacion=&v};return out }
