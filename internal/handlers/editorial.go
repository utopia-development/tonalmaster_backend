...
type articleRequest struct {
 ...
 BibliographyIDs []string `json:"bibliography_ids"`
}
...
func(h *EditorialHandler)CreateArticle(w http.ResponseWriter,r *http.Request){u,ok:=h.auth(w,r);_ = u;if !ok{return};var q articleRequest;if json.NewDecoder(r.Body).Decode(&q)!=nil||q.ID==""||q.Titulo==""||q.Fecha==""||q.Resumen==""||q.ContenidoHTML==""{writeError(w,400,"invalid_request","id, titulo, fecha, resumen and contenido_html are required");return};x,e:=articleFrom(q);if e!=nil{writeError(w,400,"invalid_date","fecha must use YYYY-MM-DD");return};x,e=h.s.CreateArticle(r.Context(),u.Role,x);if e!=nil{writeError(w,400,"article_create_failed","could not create article");return};writeJSON(w,201,x)}
func(h *EditorialHandler)UpdateArticle(w http.ResponseWriter,r *http.Request){u,ok:=h.auth(w,r);if !ok{return};var q articleRequest;if json.NewDecoder(r.Body).Decode(&q)!=nil{writeError(w,400,"invalid_request","invalid JSON");return};q.ID=r.PathValue("id");x,e:=articleFrom(q);if e!=nil{writeError(w,400,"invalid_date","fecha must use YYYY-MM-DD");return};x,e=h.s.UpdateArticle(r.Context(),u.Role,q.ID,x);if e==repository.ErrNotFound{writeError(w,404,"article_not_found","article not found");return};if e!=nil{writeError(w,400,"article_update_failed","could not update article");return};writeJSON(w,200,x)}
...
func(h *EditorialHandler)CreateCatalog(w http.ResponseWriter,r *http.Request){u,ok:=h.auth(w,r);if !ok{return};var q catalogRequest;if json.NewDecoder(r.Body).Decode(&q)!=nil||q.ID==""||q.Titulo==""{writeError(w,400,"invalid_request","id and titulo are required");return};if len(q.Detalles)==0{q.Detalles=[]byte("{}")};x,e:=h.s.CreateCatalog(r.Context(),u.Role,repository.Catalog{ID:q.ID,Titulo:q.Titulo,Descripcion:q.Descripcion,ImagenPortada:q.ImagenPortada,Detalles:q.Detalles,Visible:q.Visible});if e!=nil{writeError(w,400,"catalog_create_failed","could not create catalog");return};writeJSON(w,201,x)}
func(h *EditorialHandler)UpdateCatalog(w http.ResponseWriter,r *http.Request){u,ok:=h.auth(w,r);if !ok{return};var q catalogRequest;if json.NewDecoder(r.Body).Decode(&q)!=nil{writeError(w,400,"invalid_request","invalid JSON");return};if len(q.Detalles)==0{q.Detalles=[]byte("{}")};x,e:=h.s.UpdateCatalog(r.Context(),u.Role,r.PathValue("id"),repository.Catalog{ID:r.PathValue("id"),Titulo:q.Titulo,Descripcion:q.Descripcion,ImagenPortada:q.ImagenPortada,Detalles:q.Detalles,Visible:q.Visible});if e==repository.ErrNotFound{writeError(w,404,"catalog_not_found","not found");return};if e!=nil{writeError(w,400,"catalog_update_failed","could not update");return};writeJSON(w,200,x)}
