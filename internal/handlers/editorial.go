...
type catalogRequest struct {
 ...
 CategoriasDisponibles map[string][]string `json:"categorias_disponibles"`
 Visible *bool `json:"visible"`
}
func(h *EditorialHandler)CreateCatalog(w http.ResponseWriter,r *http.Request){u,ok:=h.auth(w,r);if !ok{return};var q catalogRequest;if json.NewDecoder(r.Body).Decode(&q)!=nil||q.ID==""||q.Titulo==""{writeError(w,400,"invalid_request","id and titulo are required");return};details,_:=json.Marshal(map[string]any{"categorias_disponibles":q.CategoriasDisponibles});visible:=true;if q.Visible!=nil{visible=*q.Visible};x,e:=h.s.CreateCatalog(r.Context(),u.Role,repository.Catalog{ID:q.ID,Titulo:q.Titulo,Descripcion:q.Descripcion,ImagenPortada:q.ImagenPortada,Detalles:details,Visible:visible});if e!=nil{writeError(w,400,"catalog_create_failed","could not create catalog");return};writeJSON(w,201,makeCatalogResponse(x,nil))}
func(h *EditorialHandler)UpdateCatalog(w http.ResponseWriter,r *http.Request){u,ok:=h.auth(w,r);if !ok{return};var q catalogRequest;if json.NewDecoder(r.Body).Decode(&q)!=nil{writeError(w,400,"invalid_request","invalid JSON");return};details,_:=json.Marshal(map[string]any{"categorias_disponibles":q.CategoriasDisponibles});visible:=true;if q.Visible!=nil{visible=*q.Visible};x,e:=h.s.UpdateCatalog(r.Context(),u.Role,r.PathValue("id"),repository.Catalog{ID:r.PathValue("id"),Titulo:q.Titulo,Descripcion:q.Descripcion,ImagenPortada:q.ImagenPortada,Detalles:details,Visible:visible});if e==repository.ErrNotFound{writeError(w,404,"catalog_not_found","not found");return};if e!=nil{writeError(w,400,"catalog_update_failed","could not update");return};writeJSON(w,200,makeCatalogResponse(x,nil))}
...
type itemRequest struct {
 ...
 AnioDescubrimiento *int `json:"año_descubrimiento"`
...
func(h *EditorialHandler)UpdateItem(w http.ResponseWriter,r *http.Request){u,ok:=h.auth(w,r);if !ok{return};var q itemRequest;if json.NewDecoder(r.Body).Decode(&q)!=nil{writeError(w,400,"invalid_request","invalid JSON");return};details,_:=json.Marshal(map[string]any{"imagen_alt":q.ImagenAlt,"descripcion":q.Descripcion,"categorias":q.Categorias,"año_descubrimiento":q.AnioDescubrimiento,"ubicacion":q.Ubicacion});x,e:=h.s.UpdateCatalogItem(r.Context(),u.Role,r.PathValue("id"),r.PathValue("item_id"),repository.CatalogItem{ID:r.PathValue("item_id"),CatalogID:r.PathValue("id"),Titulo:q.Titulo,Imagen:q.Imagen,Detalles:details});if e==repository.ErrNotFound{writeError(w,404,"item_not_found","not found");return};if e!=nil{writeError(w,400,"item_update_failed","could not update");return};writeJSON(w,200,makeItemResponse(x))}
...
func(h *EditorialHandler)CreateAd(w http.ResponseWriter,r *http.Request){u,ok:=h.auth(w,r);if !ok{return};var q adRequest;if json.NewDecoder(r.Body).Decode(&q)!=nil{writeError(w,400,"invalid_request","invalid JSON");return};if q.ID==""{writeError(w,400,"invalid_request","id is required");return};x,e:=parseAd(q,q.ID);if e!=nil{writeError(w,400,"invalid_request",e.Error());return};x,e=h.s.CreateAd(r.Context(),u.Role,x);if e!=nil{writeError(w,400,"ad_create_failed","could not create ad");return};writeJSON(w,201,makeAdResponse(x))}
...
func(h *EditorialHandler)UpdateAd(w http.ResponseWriter,r *http.Request){u,ok:=h.auth(w,r);if !ok{return};var q adRequest;if json.NewDecoder(r.Body).Decode(&q)!=nil{writeError(w,400,"invalid_request","invalid JSON");return};x,e:=parseAd(q,r.PathValue("id"));if e!=nil{writeError(w,400,"invalid_request",e.Error());return};x,e=h.s.UpdateAd(r.Context(),u.Role,x.ID,x);if e==repository.ErrNotFound{writeError(w,404,"ad_not_found","not found");return};if e!=nil{writeError(w,400,"ad_update_failed","could not update");return};writeJSON(w,200,makeAdResponse(x))}
...
func makeItemResponse(x repository.CatalogItem) itemDTOResponse { var d map[string]any;_=json.Unmarshal(x.Detalles,&d); out:=itemDTOResponse{ID:x.ID,Titulo:x.Titulo,Imagen:x.Imagen}; if v,ok:=d["imagen_alt"].(string);ok{out.ImagenAlt=&v};if v,ok:=d["descripcion"].(string);ok{out.Descripcion=&v};if v,ok:=d["categorias"].(map[string]any);ok{out.Categorias=v};if v,ok:=d["año_descubrimiento"].(float64);ok{n:=int(v);out.AnioDescubrimiento=&n};if v,ok:=d["ubicacion"].(string);ok{out.Ubicacion=&v};return out }
